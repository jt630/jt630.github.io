...
- Branch: `claude/goddard-wave-14-contact-primitives-JYI05`
...
- Wave 13 (schema conformance validator): **done** — `scripts/validate_organs.py` enforces 18 rules (R001–R018) derived directly from SCHEMA.md. Every rule has a stable rule_id, a SCHEMA.md section anchor, and a running check function — documentation and enforcement are in sync. 19 negative fixtures in `scripts/fixtures/` cover each rule (R014 uses two fixtures to exercise the cross-manifest consistency check). The validator found 9 real conformance gaps in the live library (all R009 — recovery lists ending in non-terminal `log:` or `enqueue:`, plus one nested `trigger_if_still_no_response:` construct that had no SCHEMA.md counterpart); all fixed mechanically in this wave. Green run confirmed: 37 manifests, 0 errors. CI step added (`.github/workflows/validate.yml`, triggered on every push/PR touching organs or registries, `--strict` mode, failure blocks merge). SCHEMA.md § Validation updated to name the validator as the canonical enforcement of the documented rules. Every future wave that adds a SCHEMA.md rule must add a corresponding validator rule in the same PR.
- Wave 14 (contact primitive registry): **done** — MORALITY.md gains a `## Resident-facing contact primitives` section (mirroring `## Resident-facing hold states`) with a table of registered contact primitives, their consent keys, and asserting manifests; plus a `### Adding a new resident-facing contact primitive` workflow entry. One primitive now registered: `paw_grip_offered` → `physical_catch_involuntary_fall` → `leg_fall_response`. SCHEMA.md § Action-level pointer updated with a composes-entry worked example showing `requires_consent:` alongside `only_if:`, and its scope paragraph updated to name contact primitives alongside hold states. `leg_fall_response.yaml` updated: `hand_paw_grip` composes entry now carries `requires_consent: physical_catch_involuntary_fall` as a sibling of `only_if: resident_requested_grip`. Validator extended with R019 (every `hand_*` composes entry on a registered asserter manifest carries `requires_consent: <key>` matching the registry); R019 fixture added (`scripts/fixtures/R019_contact_without_consent.yaml`). Green run confirmed: 37 manifests, 0 errors, R019 fires on fixture. Three of five consent keys now have asserters (`access_restriction`, `physical_catch_involuntary_fall` × two sites).
