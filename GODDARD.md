...
- Branch: `claude/goddard-wave-10-fallback-qhu9m`
...
- Wave 10 (fallback-grammar migration — the two-grammar collapse): **done** — the 13 remaining Wave-1-era `when:/action:/utterance:/notes:` fallback clauses in `arm_fetch_object` (4), `arm_hand_from_shelf` (5), and `arm_retrieve_dropped` (4) are translated to the canonical `trigger:/recovery:` envelope of SCHEMA § Fallback and recovery. Every clause now routes its utterance through `utterance_ref:`; older-shape `action:` verbs unfold into the closed recovery-action catalog (skill invocation, `hold:/until:`, `enqueue:`, `abort:`). Four notes-buried utterances were promoted to `voice_lines:` keys with caregiver-tuning comments preserved (`grasp_failed_handoff`, `path_blocked_hold` on fetch; `grasp_failed_shelf` on shelf; `grasp_failed_floor` on retrieve-dropped). Wave 9's handoff-confirmation audit gap closed per-manifest with option 1: each of the 3 silent `skin_speaker` composes entries now dereferences a new handoff voice_lines key (`handoff_confirmation` on fetch with `[name]` token, `handoff_confirmation` on shelf, `narrate_and_handoff` on retrieve-dropped) for caregiver sign-off. After this wave the library has one fallback grammar and zero inline `utterance:` strings — the spec's "deprecated" adjective on the older shape is now true, not aspirational.
