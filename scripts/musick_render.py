...
debug_html/musick_catalog__*.html actually contains, and write a real
per-item parser against it (see PLATFORM_FALLBACK-style comment in
auction_finder.py's fetch_musick_catalog()).

Usage (as a library, called from auction_finder.py):
    from musick_render import render_catalog_page
    html = render_catalog_page(url)   # fully rendered HTML, or None on failure
"""
...
def render_catalog_page(url, wait_selector=None):
    ...
    an optional CSS selector to appear), and return the fully rendered
    HTML. Returns None on any failure - not installed, timeout, crash -
